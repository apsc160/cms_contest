...
//
// This implementation reads and writes from/to the standard I/O streams (stdin, stdout)
// Values are printed and updated in each call to continueSuperLoop()
// Outputs:
//    - digital outputs
//    - analog outputs
...
//    - analog inputs
//
// In "CUSTOM" mode setupDAQ(-1), reads the following setup from the first line
// of standard input:
...
#ifndef DAQLIB_TERMINAL_UTILITIES_H
#define DAQLIB_TERMINAL_UTILITIES_H
...
int setupDAQ(int setupNum) {
  ...
  switch (setupNum) {
    case DAQ_SETUP_CUSTOM: {
      ...
      scanf("%d", &(__daq.digital_inputs_size));
      scanf("%d", &(__daq.digital_outputs_size));
      scanf("%d", &(__daq.analog_inputs_size));
      scanf("%d", &(__daq.analog_outputs_size));
      scanf("%d", &(__daq.display_outputs_size));
      break;
    }
